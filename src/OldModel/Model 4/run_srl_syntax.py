...
def matrix_generation_duplicate_low_dim_dependency_constituent(sentence, ntokens, max_seq_length):
    global global_input_error
    syntactic_low_dim = len(pos_list_low_dim) + 2*len(dependency_list_low_dim)
    adjacent_matrix = np.zeros((max_seq_length, max_seq_length), dtype = np.float32)
    syntactic_feature_matrix = np.zeros((max_seq_length, max_seq_length), dtype = np.float32)
    ...
    if len(tokens) == len(word_index):
        ...
        for tail, head, rel in dependency:
            ...
            try:
                ...
                global_input_error += 1
                print("exception dependency error")
                print(tokens, word_index)
                print(ntokens)
        ...
        for i, rangefromto in enumerate(range_sw):
            try:
                ...
                global_input_error += 1
                print("exception constituent error")
                print(global_input_error)
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode, use_pos, use_dependency):
    ...
    #syntax both
    # matrix_generation_duplicate_low_dim_dependency_constituent
    bias_matrix, syntactic_feature_matrix = matrix_generation_duplicate_low_dim_dependency_constituent(sentence, ntokens, max_seq_length)
        ...
        for file in listfile:  
            ...
            with open(output_predict_file,'w') as writer:
                for resultIdx, prediction in enumerate(result):
                    ...
                    # print('================================')
                    # print(f'len(prediction["predict"]) {len(prediction["predict"])}, len(prediction["input_mask"]): {len(prediction["input_mask"])}')
                    # print(f'prediction["predict"]) {prediction["predict"]}, prediction["input_mask"]: {prediction["input_mask"]}')
